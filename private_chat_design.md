...
TODO
- [x] 用户每次连接到websocket时，查询其当前是否在一个聊天室中。如果在，向其app端推送聊天室的相关状态
- [x] 房主退出房间时，应该选择下一个最早加入房间的房间内成员作为新房主，并向其推送通知；如果房主退出房间后，房间不再有人，直接解散房间
- [x] 房间无人活跃超1h时，自动解散房间
- [x] websocket应当持续向所有客户端发送房间的新增/解散情况
